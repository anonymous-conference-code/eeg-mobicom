...
class SPDIncreaseDim(nn.Module):
    def __init__(self, input_size, output_size):
        ...
        add = np.asarray(
            [0] * input_size + [1] * (output_size - input_size), dtype=np.float32
        )
...
class BiMap(nn.Module):
    def __init__(self, h, ni, no):
        ...
        self._W = functional.StiefelParameter(
            th.empty(self._h, self._ni, self._no, dtype=dtype, device=device)
        )
    ...
    def _bimap_multiplication(self, X):
        ...
        P = th.zeros(
            batch_size, self._h, self._no, self._no, dtype=X.dtype, device=X.device
        )

        for c in range(self._h):
            P[:, c, :, :] = (
                self._W[c, :, :].t().matmul(X[:, c, :, :]).matmul(self._W[c, :, :])
            )
...
class Graph_BiMap(nn.Module):
    ...
    def forward(self, X):
        ...
        if self.increase_dim:
            return self._bimap_multiplication(
                self.increase_dim(
                    th.matmul(
                        self._P, X.reshape((batch_size, channel_num, -1))
                    ).reshape((batch_size, channel_num, dim, dim))
                )
            )
        else:
            return self._bimap_multiplication(
