...
# def batch_cov(x):
#     # x is of shape [batch_size, num_channels, num_data_points]
#     # first, we compute the mean along the third dimension
#     mean = x.mean(dim=2, keepdim=True)
#     # then, we subtract the mean from the data
#     x = x - mean
#     # next, we compute the outer product along the second dimension and average over the third dimension
#     cov = x @ x.transpose(-1, -2) / (x.shape[-1] - 1)
#     # finally, we average the covariance matrices over the first dimension
#     return cov