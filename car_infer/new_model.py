...
class FilterBank:
    ...
    def filter_data(self, eeg_data, window_details={}):
        ...
        if window_details:
            n_samples = int(
                self.fs * (window_details.get("tmax") - window_details.get("tmin"))
            )
            # +1
        ...
        for i, fb in self.filter_coeff.items():
            ...
            if window_details:
                eeg_data_filtered = eeg_data_filtered[
                    :,
                    :,
                    int((window_details.get("tmin")) * self.fs) : int(
...
class EEGPredictor:
    ...
    def preprocess_data(self, data):
        ...
        filtered_data = _tensor_stack(fbank.filter_data(data).transpose(1, 0, 2, 3))
    ...
    def predict(self, data):
        ...
        # print('222222', preprocessed_data.shape)
